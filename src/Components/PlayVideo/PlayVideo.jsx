...
const PlayVideo = ({videoId}) => {
  ...
  const fetchOtherData = async () => {
    ...
    if (!apiData) return;
    const channnelData_url = `https://youtube.googleapis.com/youtube/v3/channels?part=snippet%2CcontentDetails%2Cstatistics&id=${apiData.snippet.channelId}&key=${API_KEY}`;
    await fetch(channnelData_url)
      .then((res) => res.json())
        ...
        {commentData.map((item, index) => {
         return (
          <div key={index} className="comment">
            ...
            <div>
              <h3>{item.snippet.topLevelComment.snippet.authorDisplayName} <span>{moment(item.snippet.topLevelComment.snippet.publisehsAt).fromNow()}</span></h3>
